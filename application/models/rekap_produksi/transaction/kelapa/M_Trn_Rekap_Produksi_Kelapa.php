...
class M_Trn_Rekap_Produksi_Kelapa extends CI_Model
{
    ...
    {       
        $sql = $this->db->query("SELECT
                                        COUNT(Produksi_Kelapa_Id) AS Count
                                    FROM
    ...
    function get_data($bulan, $tahun){
        ...
        return $this->db->get('tbl_trn_rencana_produksi_kelapa')->result();
    }
    ...
    function simpan_rekap($aksi, $tanggal, $jumlah){
        ...
        if($aksi == "add"){
            ...
            $this->db->insert_batch('tbl_trn_rencana_produksi_kelapa', $data_simpan);
        }else{
            for($i = 0; $i < count($tanggal); $i++){
                $arr = array(
                    'Tanggal' => date('Y-m-d', strtotime($tanggal[$i])),
            ...
            $this->db->update_batch('tbl_trn_rencana_produksi_kelapa', $data_simpan, 'Tanggal');
        }
    }
}